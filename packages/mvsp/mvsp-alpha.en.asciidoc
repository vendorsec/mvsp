...
| Apply appropriate security headers to reduce the application attack surface and limit post exploitation:
  ...
  * Limit the ability to iframe the application by enabling application-wide framing controls with X-Frame-Options
...
Sensitive application credentials and tokens should be stored separately from the application's source code repository.
